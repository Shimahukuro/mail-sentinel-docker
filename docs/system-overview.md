...
ワーカーと管理CLIは、同じPython製IMAP互換レイヤーを使用する。サーバーが`ENABLE`と`UTF8=ACCEPT`を通知した場合だけUTF-8モードを有効化し、それ以外ではIMAP Modified UTF-7を使用する。Junkフォルダーは`\Junk` Special-Use属性を優先して識別し、属性がないサーバーでは設定名へフォールバックする。この判定はプロバイダー名に依存しない。SpamAssassin はワーカーと同じ内部 Docker ネットワークに配置し、ホスト外へ直接公開しない。
