...
flowchart LR
    ...
    subgraph DockerHost["Docker ホスト"]
        Worker["メール監視・振り分け<br/>Python IMAPワーカー"]
...
ワーカーと管理CLIは、同じPython製IMAP互換レイヤーを使用する。サーバーが`ENABLE`と`UTF8=ACCEPT`を通知した場合だけUTF-8モードを有効化し、それ以外ではIMAP Modified UTF-7を使用する。Junkフォルダーは設定名との完全一致を優先し、設定したフォルダーが存在しない場合だけ、唯一の`\Junk` Special-Use属性フォルダーへフォールバックする。`\Junk`属性フォルダーが複数ある場合は、曖昧なため診断エラーとする。この判定はプロバイダー名に依存しない。SpamAssassin はワーカーと同じ内部 Docker ネットワークに配置し、ホスト外へ直接公開しない。
...
SpamAssassinの既定では、Bayes分類器が採点へ参加するために`bayes_min_ham_num=200`と`bayes_min_spam_num=200`の両方を満たす必要がある。したがって、初期学習では確認済みhamと確認済みspamをそれぞれ200件以上用意する。片方だけが最低件数へ達した状態では、学習データは保存されるが`BAYES_00`～`BAYES_99`ルールは通常の採点へ参加しない。

200件は分類器を有効にする既定の最低条件であり、精度保証ではない。学習対象は確実に分類できるメールに限定し、件数を満たす目的で曖昧なメールを混ぜてはならない。最低件数の引き下げは少量データへの過適合と誤判定の危険を伴うため、明示的な検証なしに行わない。
...
3. **ham の初期学習を実行する。** spam と同数を目安に、同じ時期の正常メールを `sa-learn --ham` 相当のジョブで学習する。件数を完全に一致させる必要はないが、既定のBayes最低条件として確認済みhamと確認済みspamをそれぞれ200件以上学習させ、一方への極端な偏りを避ける。
4. **学習状態を確認する。** `sa-learn --dump magic`の`nham`と`nspam`、失敗件数、および Bayes 分類器が利用可能な状態かを確認する。両方が200件未満の場合や`BAYES_*`ルールが発火しない場合は、Bayes判定がまだ有効ではないものとして初期スキャン結果を評価する。
...
| special-use属性 | Junkフォルダは名称指定を優先し、不在時だけ唯一の`\Junk`属性から探索する |
